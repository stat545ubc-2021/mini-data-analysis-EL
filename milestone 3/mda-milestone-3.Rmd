...
In the `msk` dataframe, the variable `Cancer Type` is of the character class. Hence, the order of the categories are by default ordered alphabetically when plotted. By reordering this variable as a factor, they can be plotted on the x-axis in ascending order of the median overall survival for each cancer type. This helps readers rapidly determine the most deadly type in the context being described by the plot. 
...
<!-------------------------- Start your work below ---------------------------->
First, we can assign each variable as related to either the patient or the sample, and store it in a new dataframe. Then, we clean the data in preparation for modelling. Categorical variables with too many levels such as `Metastatic Site` and `Primary Tumor Site` must be excluded. There are also variables that are perfectly correlated, and hence for modelling purposes only one should be kept. We then extract the significant coefficients from our multivariate linear model, with an alpha of 0.05. 
...
# store variables that are irrelevant to our model so they can be filtered
...
msk_clean[sapply(msk_clean, is.character)] <- lapply(msk_clean[sapply(msk_clean, is.character)], 
                                       as.factor)
print(msk_clean)
