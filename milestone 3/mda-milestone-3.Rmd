...
**Task Number**: 2
For the `Smoking History` variable, several rows have a missing value. As there is also a "Unknown" category for this category, NA's in this context would essentially indicate the same information, in that there is no information on the patient's smoking history.  Thus these levels can be combined when setting this variable as a factor, stored in smoking_history. 
```{r exercise1c}
msk_smoking <- msk %>%
...
write_csv(msk_os_summary, file.path("output", "msk_summary.csv"))
```
